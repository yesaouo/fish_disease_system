...
> 命名：GROD 只有一條訓練線（hard 已退役）。demo 仍有 `grod`（硬閘）/ `grod_soft` 兩種**推論模式**，但共用同一套 GROD artifacts，差別只在推論時 per-query 權重是否在 τ 二值化。on-disk 的 encoder / CEAM / bank / 腳本沿用歷史的 `_soft` 後綴（`encoder_grod_soft` / `ceah_grod_soft` / `bank_z_soft.pt` / `gpu_infer_soft.py`），prose 一律稱 GROD。
...
| 9 | 訓 GROD CEAM | Step 9 | Step 8 bank | `gpu_infer_soft.py` attribution |
...
> demo 的 `grod`（硬閘）與 `grod_soft` 模式共用同一套 `encoder_grod_soft` / `ceah_grod_soft`；硬閘只是在推論時把 per-query 權重在 τ 二值化成 {0,1}，不需要獨立訓練的 encoder / CEAM。
...
Step 4 z_joint + Step 5 distilled_global + Step 1 case_db_raw
  └─► Step 6 case_db_jointDistRawP + pool/teacher + soft_inputs + gating
        └─► Step 7 encoder_grod_soft ──► Step 8 bank_z_soft ──► Step 9 ceah_grod_soft
              (demo 的 grod 硬閘與 grod_soft 模式共用此 GROD encoder / CEAM)
```
...
- hard 閘（demo `grod` 模式）：同一顆 GROD 模型，僅把 per-query 權重在 τ（`display_thresh`）二值化成 {0,1}（`objectness > τ` 留、其餘 0）；DeepSets/CEAM 在 {0,1} 權重下 bytes-exactly 退化成硬路徑。全 0（無 lesion 過關）則 abstain。
...
Purpose：把 GROD 各步共用的 case DB、teacher/pool、`soft_inputs` 與 demo gating artifacts 一次建齊。這是 GROD encoder / CEAM 訓練前的最後 deterministic gate。
...
# 6e. soft_inputs（GROD encoder / CEAM 訓練輸入）
...
- gating 依賴 joint + distilled，不依賴 GROD encoder / CEAM，所以不應排在 Step 7/8/9 後面。

---

## 6. GROD encoder / bank / CEAM（Step 7–9）
...
Purpose：訓練 GROD 的 case evidence attribution module。
...
- 只需 Step 8 的 bank，不依賴 Step 9 的 CEAM（聚合只在 cause embedding 上做）。
...
### Step 11. 校準 demo 病因顯示門檻 `cause_score_thresh` (τ) + `cause_max_n`

Purpose：demo 病因顯示由「固定 top-n」改為**自適應**——顯示折疊後 min-max CEAM score ≥ τ
的病因、上限 `cause_max_n`、至少 1 個。τ 用 grid 搜尋，於 **fold 後**的顯示集合上量
precision / GT 覆蓋 / 平均顯示數，目標＝**平均顯示數 ≤ `target_avg` 之下最大覆蓋**（即滿足
avg≤target 的最小 τ）。與 Step 10 同性質，都是寫進 `thresholds.json` 的 demo 校準常數；γ-free（純 CEAM）。

Depends on：

- Step 6: `$ART/db/case_db_jointDistRawP` + `$ART/db/soft_inputs`（valid soft inputs）
- Step 8: `$ART/models/encoder_grod_soft/bank_z_soft.pt`
- Step 9: `$ART/models/ceah_grod_soft/best_ceah.pt`（**本步需要 CEAM**，與只吃 bank 的 Step 10 不同）
- Step 10 的 `fold_thresh`（folding 用它；故 **Step 10 須先跑**，否則 fallback 0.75）

Produces：

- `data/processed/current/thresholds.json` 內新增 `cause_score_thresh` + `cause_max_n` 鍵（保留既有 objectness / fold 鍵）

Command：

```bash
$PY -m diagnosis_model.grod.calibrate_cause_threshold
# 預設 current 樹、生產 k=3、n_max=6、target_avg=5、τ 網格 0.10:0.96:0.05。
# 換目標：--target_avg 4（更精簡）/ --n_max 8（更寬上限）。
```

Gotcha：

- 依賴 Step 9 CEAM ＋ Step 10 fold_thresh（folding 與生產同參數）；預設路徑已指向 current 樹
  （**不像** `eval_ceah_soft_paper.py` 預設指舊樹）。
- read-modify-write，**保留** objectness/fold 鍵；但重跑 `calibrate_thresholds`（整檔覆寫）會清掉，
  需重跑 Step 10 ＋ 本步補回。
- τ **無參數無關最佳**（F1 峰在 τ≈0.30 但會顯示 ~8 個病因、太多）；故用政策值 `target_avg`（預設 5）
  挑「avg≤target 的最小 τ」。fish 上得 **τ=0.25 / avg 4.83 / precision 0.638 / coverage 0.654**，
  同顯示數下 Pareto 勝固定 top-n。
- `top_n_causes`（serve/demo 參數）語意已改為**顯示上限（cap）**，預設 6＝`cause_max_n`；實際顯示數由 τ
  自適應決定。pipeline `_load_cause_thresh()` 讀，缺鍵 fallback τ=0.6 / n=6。
