...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate YOLOv11 with Consolidated Charts")
...
# =========================
def load_model():
    print(f"[INFO] Loading YOLOv11 model from {CHECKPOINT_PATH} ...")
...
def visualize_all_predictions(model, split: str, score_thresh: float = VIS_SCORE_THRESH):
    ...
    for img_id in tqdm(img_ids, desc=f"Viz {split}"):
        ...
        ax.set_axis_off()
        ax.set_title(f"YOLOv11 {split} | id={img_id}")
        plt.tight_layout()
...
def main():
    ...
    if args.checkpoint_path is None:
        CHECKPOINT_PATH = os.path.join(DATA_ROOT, "outputs", "yolov11", "train", "weights", "best.pt")
    else:
        CHECKPOINT_PATH = args.checkpoint_path

    if args.output_dir is None:
        OUTPUT_DIR = os.path.join(DATA_ROOT, "outputs", "yolov11_eval")
    else:
